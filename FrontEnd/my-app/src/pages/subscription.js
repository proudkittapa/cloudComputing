...
function Subscription(){
    ...
    const checkSubscription = () =>{
        try{
            axios.get(`http://${process.env.REACT_APP_IP}:8080/bababook/user/${userId}/subscription`)
            .then((response) => {
    ...
    const clickSubscribe = async(e) =>{
        try{
            axios.post(`http://${process.env.REACT_APP_IP}:8080/bababook/user/${userId}/subscription`, {})
            .then((response) => {
                console.log("sub",response);
                alert("subscribe")
